...
``notifies``
   **Ruby Type:** Symbol, 'Chef::Resource[String]'
   ...
   A resource may notify another resource to take action when its state changes. Sepcify the ``:action`` the ``'resource[name]'`` and the ``:timer`` for that action. A resource may notifiy more than one resource; use a ``notifies`` statement for each resource to be notified.
   ...
   .. code-block:: ruby
      ...
      notifies :run, 'resource[name]', :timer
